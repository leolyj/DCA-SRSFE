...
class BilateralConvFlex(nn.Module):
    ...
    def forward(self, features,
        ...
        for cidx in range(num_chunks):
            ...
            spread_out = splatted[batch_indices[:, None, None, None].clone(),
                                  self.feat_indices[None, :, None, None].clone(),
                                  (blur_neighbors + 1)[:, None, :, start_idx:end_idx].clone()]
        ...
        tmp_feat_blurred = feat_blurred[batch_indices[:, None, None, None].clone(), self.out_indices[None, :, None, None].clone(), out_lattice_offset[:, None, :, :].clone()]
        ...
        if self.use_bias:
            sliced = sliced.clone() + self.bias[None, :, None]
