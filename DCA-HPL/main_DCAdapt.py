...
from models import LaplaceLoss
...
def main():
    ...
    # args = cmd_args.parse_args_from_yaml('configs/train_adapt_gta_waymo.yaml')
    ...
    criterion_laplace = LaplaceLoss().cuda().to(device)
    ...
    if hasattr(args, 'resume_t') and args.resume_t:
        if osp.isfile(args.resume_t):
            logger.log("=> loading checkpoint teacher '{}'".format(args.resume_t))
            checkpoint = torch.load(args.resume_t, map_location=device)
            args.start_epoch = checkpoint['epoch']
            teacher_model.load_state_dict(checkpoint['state_dict'], strict=True)
            logger.log("=> loaded checkpoint '{}' (start epoch {}, min loss {})"
                       .format(args.resume_t, checkpoint['epoch'], checkpoint['min_loss']))
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        train_loss = train(train_loader, target_loader, student_model, teacher_model, criterion_EPE3D,
                           criterion_laplace, optimizer, teacher_optimizer, epoch, logger, device)
...
def train(train_loader, target_loader, student_model, teacher_model, criterion_epe, criterion_laplace, optimizer,
    ...
    for i, (train_batch, target_batch) in tqdm(enumerate(zip(train_loader, target_loader)),
        ...
        pc1_target, pc2_target, generated_data_target, pc1_target_2, pc2_target_2, generated_data_target_2, path_target, sf_target = target_batch  # target data
        ...
        try:
            ...
            cur_sf_target = sf_target.cuda(non_blocking=True).to(device)

            output = student_model(pc1, pc2, generated_data)  # student(source)

            output_target_student = student_model(pc1_target_2, pc2_target,
                                                  generated_data_target_2)  # student(target_2)
            output_target_teacher = teacher_model(pc1_target, pc2_target, generated_data_target)  # teacher(target)
            with torch.no_grad():
            ...
            consistency_loss = criterion_laplace(cur_pc1_target, output_target_teacher.detach(),
                                                 cur_pc2_target, cur_pc1_target_2 + output_target_student)
            # total loss
            ...
            if "CUDA error: out of memory" in str(ex) or "cuda runtime error" in str(ex) or "CUDA out of memory" in str(
                    ex) or "CUDA error" in str(ex):
                logger.log("batch idx: " + str(i) + ' path: ' + path[0])
                logger.log("target batch idx: " + str(i) + ' path: ' + path_target[0])
                logger.log("out of memory, continue")

                del pc1, pc2, sf, generated_data, pc1_target, pc2_target, generated_data_target, pc1_target_2, pc2_target_2, generated_data_target_2
                if 'output' in locals():
                    print('del output')
                    del output
                if 'output_target_student' in locals():
                    print('del output_target_student')
                    del output_target_student
                if 'output_target_teacher' in locals():
                    print('del output_target_teacher')
                    del output_target_teacher
                torch.cuda.empty_cache()
                print('empty_cache')
                gc.collect()
                print('gc.collect()')
