...
def save_checkpoint_D(state, is_best, ckpt_dir, filename='checkpoint_D.pth.tar'):
    torch.save(state, os.path.join(ckpt_dir, filename))
    if state['epoch'] % 5 == 1:
        shutil.copyfile(
            os.path.join(ckpt_dir, filename),
            os.path.join(ckpt_dir, 'checkpoint_D_'+str(state['epoch'])+'.pth.tar'))

    if is_best:
        shutil.copyfile(
            os.path.join(ckpt_dir, filename),
            os.path.join(ckpt_dir, 'model_best_D.pth.tar'))
