...
__all__ = ['waymo_target']


def augmentation(pc):
    # rotation
    angle = np.random.uniform(-0.02, 0.02)  # (0.0349 ~ 2 deg) 0.02
    ...
    rot_matrix = np.array([[cosval, 0, sinval],
                           [0, 1, 0],
                           [-sinval, 0, cosval]], dtype=np.float32)  # rotation along y axis
    # front-z, up-y

    # scale
    scale = np.diag(np.random.uniform(0.95,
                                      1.05,
                                      3).astype(np.float32))
    # shift
    shifts = np.random.uniform(-0.1,
                               0.1,
                               (1, 3)).astype(np.float32)  # 0.1  0.5
    # jitter
    jitter = np.clip(0.01 * np.random.randn(pc.shape[0], 3),
                     -0.00,
                     0.00).astype(np.float32)
    bias = shifts + jitter
...
class waymo_target(data.Dataset):
    def __init__(self,
                 train,
                 transform,
                 gen_func,
        ...
        self.save_dir_path = os.path.join(self.root, 'scene_flow_data_npz')
        ...
        print('data_len:', len(self.samples))
    ...
    def __getitem__(self, index):
        ...
        with open(fn, 'rb') as fp:
            ...
            pc1 = data['pc1']
            pc2 = data['pc2']
            sf = data['flow'] * 0.1  # velocity to scene flow
            pose_1 = data['pose1']
            pose_2 = data['pose2']
        ...
        sf_transformed = -sf[select_idx_2, :]
